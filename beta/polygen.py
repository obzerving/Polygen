...
class Polygen(inkex.EffectExtension):
        ...
        if yorient:
            ...
            for npoint in range(len(npaths[outlpath].path)):
                ...
                if npoint == 0:
                    topR = pR
                    topw = pwidth
                    lhs.extend([Move(xpos - pwidth/2,ypos)])
                    rhs.extend([Move(xpos + pwidth/2,ypos)])
                else:
                    ...
                    ypos += seglength
                    lhs.extend([Line(xpos - pwidth/2,ypos)])
                    rhs.extend([Line(xpos + pwidth/2,ypos)])
            ...
            for pcnt in range(len(lhs)):
                if (pcnt != 0) and (pcnt != (len(lhs)-1)):
                    dscore.extend(self.makescore(lhs[pcnt], rhs[pcnt], dashlength))
            ...
            for ptn in range(len(cpath.path)):
                if ptn == 0:
                    dprop.extend([Move(cpath.path[ptn].x,cpath.path[ptn].y)])
                    dwrap.extend([Move(cpath.path[ptn].x,cpath.path[ptn].y)])
                else:
                    if ptn > (len(npaths[outlpath].path)-1):
                        ...
                        dprop.extend([tabpt1])
                        dprop.extend([tabpt2])
                    dprop.extend([Line(cpath.path[ptn].x,cpath.path[ptn].y)])
                    dwrap.extend([Line(cpath.path[ptn].x,cpath.path[ptn].y)])
                    if ptn == len(cpath.path)-1:
                        ...
                        dscore.extend(self.makescore(cpath.path[ptn], cpath.path[0],dashlength))
                        dwrap.extend([ZoneClose()])
    ...
    def effect(self):
            ...
            for ptoken in elem.path.to_absolute(): # For each point in the path
                if ptoken.letter == 'M': # Starting point
                    ...
                    if sstr == None:
                        ...
                        npath.style = sstr
                    npath.path.extend([Move(ptx1,pty1)])
                else:
                    if last_letter != 'M':
                        ptx1 = ptx2
                        pty1 = pty2
                    ...
                    npath.path.extend([Line(ptx2,pty2)])
        ...
        if math.isclose(cntroffset, 0.0):
            ...
            dprop = Path()
            dprop.extend([Move(0.0,0.0)])
            dprop.extend([Line(topw0,0.0)])
            dprop.extend([Line(topw0,topw1)])
            dprop.extend([Line(0.0,topw1)])
            dprop.extend([ZoneClose()])
            self.drawline(str(dprop),npaths[outlpath].id+"lid0",layer,sstr0)
            dprop = Path()
            dprop.extend([Move(0.0,0.0)])
            dprop.extend([Line(bottomw0,0.0)])
            dprop.extend([Line(bottomw0,bottomw1)])
            dprop.extend([Line(0.0,bottomw1)])
            dprop.extend([ZoneClose()])
